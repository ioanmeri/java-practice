...
- Instance fields cannot be declared using var declaration
...
- You **cannot** access a **non-final static field** from an enum's constructor.
  ...
  - `Pets(String s) { name = prefix + s;} // not allowed`
...
- You **cannot extend an enum** from another enum or class because an enum implicitly extends `java.lang.Enum`.
  - But an **enum can implement interfaces** like `enum Pets implements java.io.Serializable`.
- Every enum extends `java.lang.Enum`
  - `Enum` declares `final` `CloneNotSupportedException`
...
- Compiler provides an enum with two public static methods automatically - `values()` and `valueOf(String)`.
  - The `values()` method returns an array of its constants and
